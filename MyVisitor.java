...
//        }
        ...
        else if(!hasJavadoc && !methName.isEmpty()  && !astNodes.isEmpty() && jdkapi != null){
            // 没有注释
            try {
                stmt = conn.prepareStatement(sqlNoComments);
                stmt.setString(1, methName);
                stmt.setString(2, bodyToken);
                stmt.setString(3, methodComplete);
                stmt.setString(4, api);
                stmt.setString(5, handleAST(astNodes));
                stmt.setString(6, jdkapi);
                stmt.execute();
            } catch (SQLException e) {
                e.printStackTrace();
            }
